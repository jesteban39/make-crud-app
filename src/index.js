...
// import {getAlias, getListModels} from './modelsNew.js'

//const ROOT = path.join('../lambda-typescript-express-sequelize-jest/src')
//const ROOT = path.join('../crud-app/src')
const ROOT = path.join('../../Axity/Axity_proyectos/proveedores-api/src')
const TEMPLETES = path.join('./src/templetes/')
const CONTROLLERS = [] // ['All', 'Create', 'Edit', 'Delete']

const getListModels = () => {
  const folders = fs.readdirSync(path.join(ROOT, '/database/models'))
  return folders.map((fileName) => fileName.replace('.ts', ''))
}

const getAlias = (modelName) => {
    return {
        Model: modelName,
        ...
        table: modelName,
        ui_Model: modelName,
        ui_model: modelName.toLowerCase(),
        ui_path: modelName,
        ui_Models: modelName,
        ui_models: modelName.toLowerCase(),
        ui_paths: modelName
      }
}
    ...
    case '[db].[dm].txt':
      return {
        ...
        lines: [
          {
            find: '<~Import~>',
            line: `import define${modelName} from './models/${modelName}'`
          },
          {
            find: '<~Declaration~>',
            line: `  define${modelName}(sequelize)`
          }
        ]
      }
    default:
...
const generateFiles = async () => {
  ...
  const filNames = ['[db].[dm].txt']

  for (const element of modelNames) {
